...
public class RedisLock {
   ...
   * 锁标志对应的key
   *
   * @desc 规则为 lock:xxxx ，如果更加细粒度使用 lock:{model_name}:{model_id}
   */
      ...
      return redisTemplate.execute(new RedisCallback<Boolean>() {
        ...
        public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
          ...
          if (result == 0 && !StringUtils.isEmpty(lockKeyLog)) {
            logger.info("[redis lock unlock][fail]Redis分布式锁，解锁{}失败！解锁时间：{}", lockKeyLog,
    ...
    return redisTemplate.execute(new RedisCallback<String>() {
      ...
      public String doInRedis(RedisConnection connection) throws DataAccessException {
        ...
        if (!StringUtils.isEmpty(lockKeyLog) && !StringUtils.isEmpty(result)) {
          logger.info("[redis lock set]获取锁{}的时间：{}", lockKeyLog, System.currentTimeMillis());
        }
  ...
  private void sleep(long millis, int nanos) {
    try {
      ...
      logger.info("[redis lock][sleep interrupted]获取分布式锁休眠被中断：", e);
    }
  }
